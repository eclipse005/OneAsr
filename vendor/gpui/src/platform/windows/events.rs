...
impl WindowsWindowInner {
    ...
    fn handle_nc_mouse_down_msg(
        ...
        if let Some(mut func) = lock.callbacks.input.take() {
            ...
            // OneAsr vendored patch (upstream Zed PR #48330): do NOT treat
            // `default_prevented` as "app handled the click". Every focus-transferring
            // click sets it (any `track_focus` element covers the titlebar), which made
            // GPUI swallow WM_NCLBUTTONDOWN before DefWindowProc — killing caption drag,
            // min/max/close and border resize for custom titlebars.
            let handled = !func(input).propagate;
            self.state.borrow_mut().callbacks.input = Some(func);
